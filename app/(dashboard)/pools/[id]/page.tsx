...
/* ============================================================
   STATUS BADGE
   ============================================================ */
function StatusBadge({ status, isFilled, isExpired }: { status: string; isFilled: boolean; isExpired: boolean }) {
  if (isFilled) return (
  ...
  const configs: Record<string, { bg: string; text: string; border: string; icon: React.ElementType }> = {
    filling: { bg: "bg-amber-500/10", text: "text-amber-400", border: "border-amber-500/20", icon: Flame },
    forged: { bg: "bg-cyan-500/10", text: "text-cyan-400", border: "border-cyan-500/20", icon: Diamond },
    active: { bg: "bg-emerald-500/10", text: "text-emerald-400", border: "border-emerald-500/20", icon: Activity },
    dormant: { bg: "bg-slate-800", text: "text-slate-500", border: "border-slate-700", icon: Satellite },
    sold: { bg: "bg-violet-500/10", text: "text-violet-400", border: "border-violet-500/20", icon: Banknote },
    dissolved: { bg: "bg-rose-500/10", text: "text-rose-400", border: "border-rose-500/20", icon: AlertTriangle },
  };
  const c = configs[status] || configs.dormant;
  const Icon = c.icon;
  return (
    <span className={cn("inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border", c.bg, c.text, c.border)}>
...
function HeroSection({ pool, vertical, colors, activeImage, setActiveImage, assetImages }: any) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-slate-800/40 bg-gradient-to-br from-[#0a0a12] to-[#050508]">
      <div className={cn("absolute top-[-20%] right-[-10%] w-[60%] h-[60%] rounded-full blur-[120px] opacity-10", colors.bg)} />
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-12 gap-0">
        ...
        <div className="lg:col-span-7 p-6 sm:p-8 lg:p-10 flex flex-col justify-between">
          <div>
            <Link href="/pools" className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-600 hover:text-cyan-400 transition-colors mb-6">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <span className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border", colors.bg, colors.text, colors.border)}>
                  <vertical.icon className="h-3 w-3" />
                  {vertical.name}
                </span>
                <div className="flex items-center gap-2 mt-1.5">
                  <StatusBadge status={pool.status} isFilled={pool.status === "forged" || pool.status === "active"} isExpired={timeLeft <= 0 && pool.status === "filling"} />
                  {pool.hallClass && (
            ...
            <div className="flex flex-wrap items-center gap-4 text-[11px] text-slate-500 font-mono mb-6">
              ...
              <span className="flex items-center gap-1.5"><Users className="h-3.5 w-3.5" /> {pool.participants}/{pool.maxParticipants} Sovereigns</span>
              ...
              <span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /> {pool.locationOptions?.[0]?.name || "Location TBD"}</span>
            </div>

            {/* Asset Value + PIR — ONLY public financials */}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="p-4 rounded-xl border border-slate-800/40 bg-slate-900/20">
                <p className="text-[9px] font-bold uppercase tracking-wider text-slate-600 mb-1">Asset Value</p>
                <p className="text-xl font-bold font-mono text-cyan-400">
                  <AnimatedCounter value={pool.assetValue || 0} prefix="$" />
                </p>
              </div>
              <div className="p-4 rounded-xl border border-slate-800/40 bg-slate-900/20">
                <p className="text-[9px] font-bold uppercase tracking-wider text-slate-600 mb-1">Capital Deployed</p>
                <p className="text-xl font-bold font-mono text-emerald-400">
                  <AnimatedCounter value={pool.committed || 0} prefix="$" />
                </p>
              </div>
              <div className="p-4 rounded-xl border border-slate-800/40 bg-slate-900/20">
                <p className="text-[9px] font-bold uppercase tracking-wider text-slate-600 mb-1">PIR Protected</p>
                <p className="text-xl font-bold font-mono text-amber-400">
                  <AnimatedCounter value={pool.pirAllocation || pool.surplus || 0} prefix="$" />
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="h-2 w-full rounded-full bg-slate-900 overflow-hidden border border-slate-800/50">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${pool.maxParticipants > 0 ? (pool.participants / pool.maxParticipants) * 100 : 0}%` }}
        ...
        <div className="lg:col-span-5 relative min-h-[300px] lg:min-h-0">
          <div className="absolute inset-0 lg:p-4">
            <div className="relative h-full rounded-2xl overflow-hidden border border-slate-800/40">
              <img src={assetImages[activeImage]} alt={pool.name} className="w-full h-full object-cover" />
              ...
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setActiveImage((prev: number) => (prev - 1 + assetImages.length) % assetImages.length)} className="h-8 w-8 rounded-lg bg-slate-900/80 border border-slate-700 flex items-center justify-center text-slate-300 hover:text-white hover:border-slate-500 transition-all backdrop-blur-sm">
                    <ChevronLeft className="h-4 w-4" />
                  </button>
                  <button onClick={() => setActiveImage((prev: number) => (prev + 1) % assetImages.length)} className="h-8 w-8 rounded-lg bg-slate-900/80 border border-slate-700 flex items-center justify-center text-slate-300 hover:text-white hover:border-slate-500 transition-all backdrop-blur-sm">
              ...
              <div className="absolute bottom-12 left-4 flex gap-2">
                {assetImages.map((img: string, i: number) => (
                  <button key={i} onClick={() => setActiveImage(i)} className={cn(
                    ...
                    i === activeImage ? "border-cyan-500/50 ring-1 ring-cyan-500/30" : "border-slate-800 opacity-60 hover:opacity-100"
                  )}>
...
function DataDashboard({ pool, colors, user }: any) {
  ...
  const participantPct = pool.maxParticipants > 0 ? ((pool.participants / pool.maxParticipants) * 100).toFixed(1) : "0";

  const stats = [
    { label: "Asset Value", value: assetValue, prefix: "$", icon: Building2, color: "cyan", sub: "Current book valuation" },
    { label: "Capital Deployed", value: committed, prefix: "$", icon: Coins, color: "emerald", sub: "Total sovereign commitments" },
    { label: "PIR Reserve", value: pir, prefix: "$", icon: Shield, color: "amber", sub: "Protocol protection layer" },
    { label: "Sovereigns", value: pool.participants || 0, prefix: "", icon: Users, color: "violet", sub: `${participantPct}% of capacity` },
  ];
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {stats.map((s, i) => {
        ...
        return (
          ...
          >
            <div className={cn("absolute top-0 right-0 h-16 w-16 rounded-full blur-3xl opacity-10", c.bg)} />
            <div className="relative">
              <div className="flex items-center gap-2 mb-3">
                <Icon className={cn("h-3.5 w-3.5", c.text)} />
                <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-600">{s.label}</span>
              </div>
              <p className={cn("text-xl sm:text-2xl font-bold font-mono", c.text)}>
    ...
    <GlassCard accent={colors.color} glow>
      <div className="p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Location Options</span>
          <div className="flex-1 h-px bg-gradient-to-r from-slate-800 to-transparent" />
          <span className="text-[10px] font-mono text-slate-700">{options.length} sites</span>
        </div>
              ...
              {loc.isSelected && (
                <div className="absolute top-2 right-2">
                  <span className="flex h-2 w-2">
                    ...
                    <span className={cn("relative inline-flex rounded-full h-2 w-2", colors.bg)} />
                  </span>
                </div>
              )}
              <div className="flex items-start gap-3">
                <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center border shrink-0", loc.isSelected ? colors.bg : "bg-slate-800 border-slate-700")}>
                  <MapPin className={cn("h-4 w-4", loc.isSelected ? colors.text : "text-slate-600")} />
                </div>
                <div className="min-w-0">
                  <p className="text-xs font-bold text-white truncate">{loc.name}</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">{loc.address || "Address pending"}</p>
                  {loc.lat && loc.lng && (
                  ...
                  {loc.votes > 0 && (
                    <p className="text-[9px] text-slate-600 mt-1">{loc.votes} votes</p>
                  )}
                </div>
              </div>
            </motion.div>
      ...
      <div className="p-6 flex items-center gap-4">
        ...
        <div>
          <p className="text-sm font-bold text-white">
            {pool.status === "forged" ? "Pool Forged — Hall Unlocked" :
             pool.status === "active" ? "Pool Active — Revenue Flowing" :
             "Pool Closed"}
          </p>
    ...
    <GlassCard accent="cyan" glow>
      <div className="p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-400/80">Commit Terminal</span>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="p-3 rounded-lg border border-slate-800/40 bg-slate-900/20">
            ...
            <p className="text-sm font-bold font-mono text-white">${pool.minCommitment || 1}</p>
          </div>
          <div className="p-3 rounded-lg border border-slate-800/40 bg-slate-900/20">
            <p className="text-[9px] text-slate-600 uppercase tracking-wider font-bold">Max Commit</p>
            <p className="text-sm font-bold font-mono text-white">${pool.maxCommitment || "∞"}</p>
          </div>
          <div className="p-3 rounded-lg border border-slate-800/40 bg-slate-900/20">
            <p className="text-[9px] text-slate-600 uppercase tracking-wider font-bold">Your Balance</p>
            <p className="text-sm font-bold font-mono text-emerald-400">${user?.ledgerBalance?.toLocaleString() || "0"}</p>
          </div>
          <div className="p-3 rounded-lg border border-slate-800/40 bg-slate-900/20">
            <p className="text-[9px] text-slate-600 uppercase tracking-wider font-bold">Est. Ownership</p>
    ...
    <GlassCard accent="cyan" glow>
      <div className="p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Sovereign Access Granted</h3>
            <p className="text-[10px] text-slate-500">You are a sovereign owner of this asset</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="p-3 rounded-xl border border-slate-800/40 bg-slate-900/20">
            <p className="text-[9px] text-slate-600 uppercase tracking-wider font-bold">Your Commitment</p>
            <p className="text-lg font-bold text-white font-mono">${userOwnership.amountCommitted?.toLocaleString() || "0"}</p>
          </div>
          <div className="p-3 rounded-xl border border-slate-800/40 bg-slate-900/20">
            <p className="text-[9px] text-slate-600 uppercase tracking-wider font-bold">Ownership</p>
            <p className="text-lg font-bold text-cyan-400 font-mono">{userOwnership.ownershipPercent?.toFixed(2) || "0"}%</p>
          </div>
          <div className="p-3 rounded-xl border border-slate-800/40 bg-slate-900/20">
            <p className="text-[9px] text-slate-600 uppercase tracking-wider font-bold">Monthly Dividend</p>
            <p className="text-lg font-bold text-emerald-400 font-mono">${userOwnership.dividendPreview?.monthly?.toLocaleString() || "0"}</p>
          </div>
          <div className="p-3 rounded-xl border border-slate-800/40 bg-slate-900/20">
            <p className="text-[9px] text-slate-600 uppercase tracking-wider font-bold">Est. Annual Yield</p>
            <p className="text-lg font-bold text-amber-400 font-mono">{userOwnership.dividendPreview?.yield || "0"}%</p>
          </div>
        </div>
    ...
    <GlassCard accent="amber">
      <div className="p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-400/80">Community Reviews</span>
          <div className="flex-1 h-px bg-gradient-to-r from-amber-500/20 to-transparent" />
          <span className="text-[10px] font-mono text-slate-700">{reviewStats.total} entries</span>
        </div>

        {user && (
          <form onSubmit={handleSubmit} className="mb-6 p-4 rounded-xl border border-slate-800/40 bg-slate-900/20">
            <div className="flex items-center gap-1 mb-3">
              {[1, 2, 3, 4, 5].map((s) => (
                <button key={s} type="button" onClick={() => setRating(s)} className="p-0.5">
                  <Star className={cn("h-5 w-5 transition-colors", s <= rating ? "text-amber-400 fill-amber-400" : "text-slate-700")} />
                </button>
              ))}
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-300">{review.user?.displayName || "Anonymous"}</p>
                      <div className="flex items-center gap-0.5">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star key={s} className={cn("h-3 w-3", s <= review.rating ? "text-amber-400 fill-amber-400" : "text-slate-700")} />
                        ))}
                      </div>
                    </div>
                  </div>
                  <span className="text-[9px] font-mono text-slate-700">{timeAgo(review.createdAt)}</span>
                </div>
                {review.title && <p className="text-xs font-semibold text-slate-300 mb-1">{review.title}</p>}
...
export default function PoolDetailPage() {
    ...
    async function fetchData() {
      try {
        ...
        if (reviewData.success) {
          ...
          setReviewStats({ total: reviewData.total || 0, average: reviewData.averageRating || 0 });
        }
      } catch (err) {
  ...
  async function handleCommit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.success) {
        setMessage(`Committed $${commitAmount}. PAC token: ${data.pacToken || "Pending"}`);
  ...
  async function handleReviewSubmit({ poolId, rating, content }: any) {
    ...
    try {
      ...
      if (data.success) {
        ...
        setReviewStats((prev) => ({
          total: prev.total + 1,
          average: data.review?.rating ? Math.round(((prev.average * prev.total + data.review.rating) / (prev.total + 1)) * 10) / 10 : prev.average,
        }));
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-[#050508] text-slate-100 selection:bg-cyan-500/20 selection:text-cyan-100">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          />
          <SovereignAccess pool={pool} userOwnership={userOwnership} colors={colors} />
        </div>
