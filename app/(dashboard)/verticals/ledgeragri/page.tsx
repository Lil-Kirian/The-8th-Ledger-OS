        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-xs font-mono text-emerald-400 uppercase tracking-[0.2em]">
                Vertical Command
              </span>
            </div>
            ...
            <p className="text-slate-400 mt-2 text-sm max-w-xl">
              Sovereign ownership of agricultural assets — crop farms,
              greenhouses, livestock, dairy, aquaculture, and apiaries. Class
              III Active. Hall governs operations. 8th Ledger manages the
        ...
        <GlowCard accent="emerald" className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-semibold text-white">
                Global Yield Velocity
              </h3>
              <p className="text-xs text-slate-400 mt-1">
                12-month production tonnage across all LedgerAgri assets
              </p>
            </div>
            <div className="flex gap-2">
              {["1M", "3M", "6M", "1Y", "ALL"].map((range) => (
                <button
                  key={range}
                  className={`px-3 py-1 rounded-lg text-xs font-mono border transition-all ${range === "1Y" ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-300" : "bg-transparent border-slate-700 text-slate-500 hover:border-slate-500"}`}
                >
                  {range}
                </button>
              ))}
          ...
          <div className="h-56 w-full">
            ...
            >
              ...
              />
              <path
                d={`M0,${220 - ((YIELD_HISTORY[0] - 3000) / 3000) * 220} ${YIELD_HISTORY.map(
                  (v, i) => {
                    const x = (i / (YIELD_HISTORY.length - 1)) * 800;
                    const y = 220 - ((v - 3000) / 3000) * 220;
                    return `L${x},${y}`;
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Community vote on next cultivation site
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {LOCATIONS.map((loc) => (
              <GlowCard
                key={loc.id}
                accent={selectedLocation === loc.id ? "emerald" : "slate"}
                className={
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  ...
                  <div className="p-2 rounded-lg bg-slate-900/50 border border-slate-800 text-center">
                    ...
                    <div className="text-sm font-mono font-bold text-violet-300">
                      {loc.elevation}m
                    </div>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Community decides what we cultivate next season
                </p>
              </div>
            </div>
            <span className="text-xs font-mono text-slate-500">
              {CROP_VOTES.reduce((s, c) => s + c.votes, 0)} total votes
            </span>
          </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-500 font-mono">
                          {crop.season} • {crop.projectedYield}
                        </div>
                      </div>
                    </div>
        ...
        <GlowCard accent="amber" className="mb-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Machinery available to Hall owners
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {EQUIPMENT.map((eq) => (
              <div
                key={eq.id}
                className="p-4 rounded-xl bg-slate-900/40 border border-slate-800 hover:border-amber-500/20 transition-all group"
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
            ...
            {FARMS.map((farm) => (
              <GlowCard key={farm.id} accent="emerald">
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  ...
                  <div className="p-2 rounded-lg bg-slate-900/50 border border-slate-800 text-center">
                    ...
                    <div className="text-sm font-bold text-amber-400 font-mono">
                      ${farm.monthlyIncome.toLocaleString()}
                    </div>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <div className="text-xs text-slate-500">
                    Current crop:{" "}
                    <span className="text-emerald-300">{farm.crop}</span>
                  </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 flex-1">
              <div>
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  ...
                  monthly after the 8th Ledger Tithe (20%) and payroll costs.
                </p>
              </div>
              <div>
                <h4 className="text-sm font-semibold text-white mb-2">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  LedgerAgri is Class III Active — the hall governs daily
                  operations including crop selection, inventory orders,
                  staffing levels, and equipment rental. The 8th Ledger handles
                  payroll execution, vendor contracts, and worker management
                  through the Forge Ledger. Any hall can vote to enable
