...
const GlowCard = ({ children, className = "", accent = "blue" }: { children: React.ReactNode; className?: string; accent?: "blue" | "cyan" | "gold" | "slate" | "emerald" | "amber" }) => {
  const accentMap = {
...
const StatusBadge = ({ status }: { status: string }) => {
  const map: Record<string, string> = {
    filling: "bg-blue-500/10 text-blue-400 border-blue-500/30",
...
export default function LedgerPropPage() {
  ...
  useEffect(() => {
    async function fetchLedgerProp() {
      try {
  ...
  return (
    <div className="min-h-screen bg-[#050508] text-slate-100 selection:bg-blue-500/30 selection:text-blue-100">
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-[-15%] left-[-10%] w-[45%] h-[45%] bg-blue-500/5 rounded-full blur-[120px]" />
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-6 py-10">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="h-2 w-2 rounded-full bg-blue-400 animate-pulse shadow-[0_0_10px_rgba(59,130,246,0.6)]" />
              <span className="text-xs font-mono text-blue-400 uppercase tracking-[0.2em]">Class I — Passive Infrastructure</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight bg-gradient-to-r from-white via-blue-100 to-blue-200 bg-clip-text text-transparent flex items-center gap-4">
              <span className="text-5xl">🏠</span> LedgerProp
            </h1>
            <p className="text-slate-400 mt-2 text-sm max-w-xl">
              Own real estate. Residential apartments, commercial towers, retail malls, industrial warehouses, student housing, senior living. 
              8th Ledger manages everything — tenant screening, rent collection, maintenance, insurance. You own the asset. You earn the rent — forever.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <button className="group relative px-6 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold text-sm overflow-hidden transition-all hover:shadow-[0_0_30px_rgba(59,130,246,0.4)] hover:scale-105 active:scale-95">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-blue-500/30 bg-gradient-to-br from-blue-500/10 via-[#0a0a14] to-transparent p-8 mb-8">
          <div className="absolute -right-40 -top-40 h-96 w-96 rounded-full bg-blue-500/10 blur-[120px]" />
          <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <Crown className="h-5 w-5 text-blue-400" />
                <span className="text-xs font-bold uppercase tracking-wider text-blue-400/80">Perpetual Asset Contract</span>
              </div>
              <h2 className="text-2xl font-bold text-white mb-3">Own the Building. Earn from Every Tenant.</h2>
              <p className="text-sm text-slate-400 leading-relaxed mb-4">
                In LedgerProp, every committer becomes a co-owner through PACs. When the pool fills, the 8th Ledger acquires the property, forms the legal SPV, and activates insurance. Revenue from leases flows to the hall treasury. The 20% tithe sustains the protocol. The rest is yours — split by ownership percentage, distributed monthly.
              </p>
              <div className="flex flex-wrap gap-3">
                <span className="px-3 py-1.5 rounded-lg bg-blue-500/10 text-blue-300 border border-blue-500/20 text-xs font-mono">All = Owners</span>
            ...
            <div className="relative">
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-2xl bg-slate-900/50 border border-slate-800 text-center">
                  <div className="text-3xl font-bold text-blue-400 font-mono mb-1">100%</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          <GlowCard accent="blue">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-mono text-blue-400 uppercase tracking-wider">Asset Value</span>
              <Building2 className="h-4 w-4 text-blue-400/50" />
            </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search properties by name, country, or type..."
              className="w-full rounded-xl border border-slate-700 bg-slate-900/50 pl-9 pr-4 py-2.5 text-sm text-white placeholder:text-slate-600 outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20"
            />
          </div>
          <button
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <div className="h-10 w-10 animate-spin rounded-full border-2 border-blue-500 border-t-transparent shadow-[0_0_15px_rgba(59,130,246,0.3)]" />
          </div>
        ) : filteredPools.length === 0 ? (
          <GlowCard accent="slate" className="text-center py-16">
            <Home className="mx-auto h-12 w-12 text-slate-700 mb-4" />
            <p className="text-lg text-slate-400 font-semibold">No properties found</p>
            <p className="text-xs text-slate-600 mt-2">
              {searchQuery ? "Try a different search term" : "Check back soon for new real estate listings"}
            </p>
          </GlowCard>
        ) : (
            ...
            <AnimatePresence mode="popLayout">
              {filteredPools.map((pool) => {
                ...
                return (
                  ...
                  >
                    <GlowCard accent="blue" className="h-full flex flex-col">
                      ...
                      <p className="text-xs text-slate-500 line-clamp-2 mb-3">{pool.description || "Premium real estate asset."}</p>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-400 text-[10px] border border-slate-700 flex items-center gap-1">
                          <Building2 className="h-3 w-3" /> Residential
                        </span>
                        <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-400 text-[10px] border border-slate-700 flex items-center gap-1">
                          <Warehouse className="h-3 w-3" /> Commercial
                        </span>
                        <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-400 text-[10px] border border-slate-700 flex items-center gap-1">
                          <Globe className="h-3 w-3" /> LedgerProp
                        </span>
                      </div>
                      ...
                      <div className="mt-auto p-4 rounded-xl bg-slate-900/40 border border-slate-800">
                        <div className="flex items-baseline justify-between mb-2">
                          <div>
                            <p className="text-[10px] uppercase tracking-wider text-blue-400/60 font-mono">Target</p>
                        ...
                        <div className="mb-2">
                          <div className="flex items-center justify-between text-[10px] mb-1">
                            <span className="text-blue-400/80 font-mono">{fillPct.toFixed(1)}% to target</span>
                            <span className="text-slate-600 font-mono">${pool.committed.toLocaleString()}</span>
                          </div>
                          <div className="h-2 w-full rounded-full bg-slate-800 overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-blue-500 to-cyan-400 rounded-full transition-all" style={{ width: `${fillPct}%` }} />
                          </div>
                        </div>
                      ...
                      <Link
                        ...
                        className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-blue-500/10 border border-blue-500/30 py-2.5 text-xs font-bold uppercase tracking-wider text-blue-400 hover:bg-blue-500/20 transition-all"
                      >
                        View Asset
        ...
        <GlowCard accent="blue" className="mt-8">
          <div className="flex items-start gap-4">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-500/10 border border-blue-500/30">
              <Radio className="h-5 w-5 text-blue-400" />
            </div>
            <div className="grid gap-4 sm:grid-cols-2 flex-1">
              <div>
                <h4 className="text-sm font-semibold text-white mb-2">How LedgerProp Works</h4>
                <p className="text-xs text-slate-400 leading-relaxed">
                  Each asset shows its true acquisition cost and pool target. When the pool fills, the 8th Ledger acquires the property, forms the legal SPV, and activates insurance. 
                  Revenue from leases flows to the hall treasury. The 8th Ledger Tithe (20%) covers protocol operations. The remainder splits by PAC ownership percentage — monthly, automatically.
                </p>
              </div>
              <div>
                <h4 className="text-sm font-semibold text-white mb-2">Class I — Passive</h4>
                <p className="text-xs text-slate-400 leading-relaxed">
                  8th Ledger manages everything. Minimal hall input. No workers hired by the hall — only contracted vendors for property management. 
                  You vote on lease terms and major repairs. The 8th Ledger handles tenant screening, rent collection, maintenance, and insurance. Your PAC earns forever.
                </p>
              </div>
            </div>
          </div>
