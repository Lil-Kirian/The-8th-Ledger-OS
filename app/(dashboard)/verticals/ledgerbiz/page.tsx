...
import {
  TrendingUp, ArrowRight, Briefcase, Timer, Users, Target, Flame,
  Search, SlidersHorizontal, ChevronDown, Store, MapPin, Zap,
...
const GlowCard = ({ children, className = "", accent = "orange" }: { children: React.ReactNode; className?: string; accent?: "orange" | "cyan" | "gold" | "purple" | "red" | "slate" | "emerald" | "amber" }) => {
  const accentMap = {
    orange: "from-orange-500/20 to-red-500/20 border-orange-500/30 shadow-orange-500/10",
...
const StatusBadge = ({ status }: { status: string }) => {
  const map: Record<string, string> = {
    filling: "bg-orange-500/10 text-orange-400 border-orange-500/30",
...
const ImageSkeleton = () => (
  <div className="flex h-full w-full items-center justify-center bg-slate-900/80">
    <Store className="h-12 w-12 text-slate-700 animate-pulse" />
  </div>
);

export default function LedgerBizPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050508] text-slate-100 selection:bg-orange-500/30 selection:text-orange-100">
      {/* Background */}
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-[-15%] left-[-10%] w-[45%] h-[45%] bg-orange-500/5 rounded-full blur-[120px]" />
        <div className="absolute bottom-[-15%] right-[-10%] w-[45%] h-[45%] bg-red-500/5 rounded-full blur-[120px]" />
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-6 py-10">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          ...
          <div className="flex items-center gap-3">
            <button className="group relative px-6 py-3 rounded-xl bg-gradient-to-r from-orange-600 to-red-600 text-white font-semibold text-sm overflow-hidden transition-all hover:shadow-[0_0_30px_rgba(234,88,12,0.4)] hover:scale-105 active:scale-95">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search supermarkets, restaurants, coworking, logistics..."
              className="w-full rounded-xl border border-slate-700 bg-slate-900/50 pl-9 pr-4 py-2.5 text-sm text-white placeholder:text-slate-600 outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/20"
            />
          </div>
          <button
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <div className="h-10 w-10 animate-spin rounded-full border-2 border-orange-500 border-t-transparent shadow-[0_0_15px_rgba(234,88,12,0.3)]" />
          </div>
                  ...
                  >
                    <GlowCard accent="orange" className="h-full flex flex-col">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        ...
                        <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-400 text-[10px] border border-slate-700 flex items-center gap-1">
                          <UtensilsCrossed className="h-3 w-3" /> Food Service
                        </span>
                        <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-400 text-[10px] border border-slate-700 flex items-center gap-1">
                          <Store className="h-3 w-3" /> LedgerBiz
                        </span>
                      </div>
                      ...
                      <Link
                        ...
                        className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-orange-500/10 border border-orange-500/30 py-2.5 text-xs font-bold uppercase tracking-wider text-orange-400 hover:bg-orange-500/20 transition-all"
                      >
                        View Asset
        ...
        {/* LedgerBiz Info Footer */}
        <GlowCard accent="orange" className="mt-8">
          <div className="flex items-start gap-4">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-orange-500/10 border border-orange-500/30">
