...
export async function GET(req: NextRequest) {
  try {
    ...
    if (minPrice) {
      where.price = { ...(where.price as Prisma.IntFilter | undefined), gte: parseInt(minPrice, 10) };
    }
    if (maxPrice) {
      where.price = { ...(where.price as Prisma.IntFilter | undefined), lte: parseInt(maxPrice, 10) };
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hallId || !title || price == null || quantity == null) {
      return NextResponse.json(
        { success: false, error: "hallId, title, price, and quantity are required" },
        { status: 400 }
      );
    }
    ...
    if (!Number.isFinite(numPrice) || numPrice <= 0 || !Number.isFinite(numQty) || numQty <= 0) {
      return NextResponse.json(
        { success: false, error: "Price and quantity must be positive numbers" },
        { status: 400 }
      );
    }
    ...
    if (!hall.inventoryEnabled) {
      return NextResponse.json(
        { success: false, error: "Hall does not have inventory enabled. Vote to enable first." },
        { status: 403 }
      );
    }
    ...
    if (!isFounder) {
      ...
      if (!ownership) {
        return NextResponse.json(
          { success: false, error: "You must be an owner of this hall to list inventory" },
          { status: 403 }
        );
      }
    }
