...
export const poolsApi = {
  list: (params?: { vertical?: string; status?: string; user?: string }) => {
    ...
    return fetcher<{ pools: any[] }>(`/pools${qs}`, { method: "GET" });
  },

  create: (payload: PoolPayload) =>
    fetcher<{ pool: any }>("/pools", { method: "POST", body: JSON.stringify(payload) }),

  commit: (payload: CommitPayload) =>
    fetcher<{ pool: any; message: string }>("/pools", {
      method: "PATCH",
...
export const exchangeApi = {
  state: (type: "all" | "book" | "trades" | "bonding" = "all") =>
    fetcher<{ bonding?: any; orders?: any[]; trades?: any[] }>(
      `/exchange/orders?type=${type}`,
      { method: "GET" }
    ),

  placeOrder: (payload: OrderPayload) =>
    fetcher<{ order: any; trades?: any[]; message: string }>("/exchange/orders", {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  cancelOrder: (orderId: string) =>
    fetcher<{ order: any; message: string }>(`/exchange/orders?id=${orderId}`, {
      method: "DELETE",
    }),
};
...
export const oracleApi = {
  getForecasts: (status?: "active" | "locked" | "resolved" | "all") =>
    fetcher<{ forecasts: any[] }>(`/oracle/forecasts?status=${status || "all"}`, { method: "GET" }),

  getStanding: () =>
    fetcher<{ standing: any }>("/oracle/standing", { method: "GET" }),

  getLeaderboard: () =>
    fetcher<{ leaderboard: any[] }>("/oracle/leaderboard", { method: "GET" }),

  predict: (payload: OracleForecastPayload) =>
    fetcher<{ prediction: any; message: string }>("/oracle/forecasts", {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  resolveForecast: (payload: { forecastId: string; winningVertical: string; winningCountry: string }) =>
    fetcher<{ resolved: any; message: string }>(`/oracle/forecasts`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    }),
};
...
export const knotApi = {
  ...
  recordReferral: (payload: { fromLedgerId: string; activityType: "commit" | "win" | "forge"; amount: number }) =>
    fetcher<{ rewards: any[]; message: string }>("/knot", {
      method: "POST",
...
export const treasuryApi = {
  ...
  withdraw: (payload: WithdrawPayload) =>
    fetcher<{ withdrawals: any[]; message: string }>("/treasury", {
      method: "POST",
  ...
  allocatePir: (payload: { poolId: string; poolName?: string; totalCommitted: number; trueCost: number }) =>
    fetcher<{ pirAllocations: any[]; message: string }>("/treasury", {
      method: "POST",
  ...
  distributeRevenue: (payload: { hallId: string; poolId: string; grossRevenue: number; payrollAmount?: number }) =>
    fetcher<{ distribution: any; message: string }>("/treasury", {
      method: "POST",
...
export const meridianApi = {
  getCurrentCycle: () =>
    fetcher<{ cycle: any }>("/meridian/cycle", { method: "GET" }),

  getCycleDetails: (cycleId: string) =>
    fetcher<{ cycle: any }>(`/meridian/cycle/${cycleId}`, { method: "GET" }),

  getCyclePools: (cycleId: string) =>
    fetcher<{ pools: any[] }>(`/meridian/cycle/${cycleId}/pools`, { method: "GET" }),

  vote: (payload: { cycleId: string; poolId: string }) =>
    fetcher<{ vote: any; message: string }>(`/meridian/cycle/${payload.cycleId}/vote`, {
      method: "POST",
...
export const agoraApi = {
  getSuggestions: (filters?: { status?: string; continent?: string; page?: number }) => {
    ...
    return fetcher<{ suggestions: any[]; total: number }>(`/agora/suggestions${qs}`, { method: "GET" });
  },

  createSuggestion: (payload: AgoraSuggestionPayload) =>
    fetcher<{ suggestion: any; message: string }>("/agora/suggestions", {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  voteSuggestion: (suggestionId: string, direction: "up" | "down") =>
    fetcher<{ suggestion: any; message: string }>(`/agora/suggestions/${suggestionId}/vote`, {
      method: "POST",
      body: JSON.stringify({ direction }),
    }),

  getQA: (page?: number) =>
    fetcher<{ questions: any[] }>(`/agora/qa?page=${page || 1}`, { method: "GET" }),

  askQuestion: (payload: { question: string }) =>
    fetcher<{ qa: any; message: string }>("/agora/qa", {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  getRelay: (page?: number) =>
    fetcher<{ feed: any[] }>(`/agora/relay?page=${page || 1}`, { method: "GET" }),

  getPulse: () =>
    fetcher<{ stats: any }>("/agora/pulse", { method: "GET" }),
};
...
export const marketplaceApi = {
  getOwnershipListings: (filters?: { hallId?: string; verticalId?: string; page?: number }) => {
    const qs = filters ? `?${new URLSearchParams(filters as Record<string, string>).toString()}` : "";
    return fetcher<{ listings: any[]; total: number }>(`/marketplace/ownership${qs}`, { method: "GET" });
  },

  createOwnershipListing: (payload: OwnershipListingPayload) =>
    fetcher<{ listing: any; message: string }>("/marketplace/ownership", {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  buyOwnership: (listingId: string) =>
    fetcher<{ order: any; message: string }>(`/marketplace/ownership/${listingId}/buy`, {
      method: "POST",
    }),
  ...
  getInventory: (filters?: { hallId?: string; page?: number }) => {
    const qs = filters ? `?${new URLSearchParams(filters as Record<string, string>).toString()}` : "";
    return fetcher<{ items: any[]; total: number }>(`/marketplace/inventory${qs}`, { method: "GET" });
  },

  buyInventory: (payload: InventoryBuyPayload) =>
    fetcher<{ order: any; message: string }>(`/marketplace/inventory/${payload.inventoryId}/buy`, {
      method: "POST",
...
export const hallApi = {
  getStream: (hallId: string, page?: number) =>
    fetcher<{ posts: any[] }>(`/halls/${hallId}/stream?page=${page || 1}`, { method: "GET" }),

  createStreamPost: (hallId: string, payload: { type: "PROPOSAL" | "REPORT" | "APPEAL"; title: string; content: string; proposalId?: string }) =>
    fetcher<{ post: any; message: string }>(`/halls/${hallId}/stream`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  getCabinet: (hallId: string) =>
    fetcher<{ cabinet: any }>(`/halls/${hallId}/cabinet`, { method: "GET" }),

  electCabinet: (hallId: string, payload: { speakerId?: string; treasurerId?: string; wardenId?: string; scribeId?: string }) =>
    fetcher<{ cabinet: any; message: string }>(`/halls/${hallId}/cabinet/elect`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  getForge: (hallId: string) =>
    fetcher<{ forge: any }>(`/halls/${hallId}/forge`, { method: "GET" }),

  getLedger: (hallId: string) =>
    fetcher<{ updates: any[] }>(`/halls/${hallId}/ledger`, { method: "GET" }),

  getVault: (hallId: string) =>
    fetcher<{ documents: any[] }>(`/halls/${hallId}/vault`, { method: "GET" }),

  getClosure: (hallId: string) =>
    fetcher<{ closure: any }>(`/halls/${hallId}/closure`, { method: "GET" }),
};